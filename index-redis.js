const { Client, IntentsBitField, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, ChannelType, PermissionsBitField, ModalBuilder, TextInputBuilder, TextInputStyle, UserSelectMenuBuilder, InviteGuild } = require('discord.js')
...
client.on('voiceStateUpdate', async (OldVoice, NewVoice) => {
    if (BotConfig.ChannelsId.includes(NewVoice.channelId)){
        await NewVoice.guild.channels.create({
            ...
            await db.set(`Temporary_${Channel.id}_${OldVoice.member.user.id}`, Channel.id)
            await NewVoice.member.voice.setChannel(Channel)
        })
    }

    const interval = setInterval(async () => {
            if(await db.get(`Temporary_${OldVoice.channelId}_${OldVoice.member.user.id}`) == null) return clearInterval(interval);
            delChannel: if (OldVoice.channelId !== null) {
                    if (OldVoice.channel.members.filter(x => !x.user.bot).size == 0) {
                        ...
                        await db.del(`Temporary_${OldVoice.channelId}_${OldVoice.member.user.id}`);
                        // console.log('testhapus')
...
client.on('interactionCreate', async Interaction => {
    if (Interaction.isButton()) {
        ...
        const Data = await db.get(`Temporary_${Channel.id}_${Interaction.user.id}`)
